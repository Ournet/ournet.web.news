	...
	topicByUniqueName(currentCulture, uniqueName)
		.then(function(topic) {
			...
			var feed = new Rss({
				...
				feed_url: canonical,
				site_url: 'http://' + config.host + links.topic(topic.slug, { ul: currentCulture.lang }),
				language: currentCulture.lang,
				pubDate: new Date(),
				ttl: cacheTtl,
				...
				.then(function(stories) {
					stories.forEach(function(story) {
						feed.item({
							...
							url: 'http://' + config.host + links.story(story.uniqueName, story.id, { ul: currentCulture.lang }),
							guid: 'story-' + story.id,
...
route.get('/:ul' + utils.localePrefix + '?/rss/stories.xml', function(req, res, next) {
	...
	var feed = new Rss({
		title: __('stories'),
		// description: description,
		feed_url: 'http://' + config.host + links.rss.stories({ ul: currentCulture.lang }),
...
route.get('/:ul' + utils.localePrefix + '?/rss/stories/important.xml', function(req, res, next) {
	...
	var feed = new Rss({
		...
		// description: description,
		feed_url: 'http://' + config.host + links.rss.stories.important({ ul: currentCulture.lang }),
		site_url: 'http://' + config.host + links.important({ ul: currentCulture.lang }),
		language: currentCulture.lang,
		pubDate: new Date(),
		ttl: cacheTtl,
		generator: config.name
